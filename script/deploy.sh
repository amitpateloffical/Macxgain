...
create_backup() {
    ...
    if systemctl is-active --quiet mysql; then
        ...
        mysqldump -h localhost -u root -p"Kabirisgod@7354$" trading > "$backup_path/database.sql"
    else
...
rollback() {
    ...
    if [ -f "$backup_path/database.sql" ]; then
        ...
        mysql -h localhost -u root -p"Kabirisgod@7354$" trading < "$backup_path/database.sql"
    fi
