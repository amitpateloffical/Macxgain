  ...
  <div class="dashboard">
    <div class="list__page">
      ...
      <div class="list__title">
        ...
        <b-button v-if="!isAdmin" variant="primary" :disabled="loading" @click="openRequestModal()">
          Create Request
        </b-button>
        <b-button variant="primary" class="mb-0 ml-md-1 basicButton" v-ripple.400="'rgba(113, 102, 240, 0.15)'"
          ...
          <b-row>
            <b-col md="4">
              <b-form-group label="Transaction ID" label-for="transaction_id">
                <b-form-input v-model="search.transaction_id" id="transaction_id" placeholder="Enter Transaction ID" />
              </b-form-group>
            </b-col>
            <b-col md="4">
              <b-form-group label="Status" label-for="status">
                <b-form-select v-model="search.status" :options="statusOptions" id="status">
                  <template #first>
            ...
            <b-col md="4" v-if="isAdmin">
              <b-form-group label="Recipient" label-for="recipient">
                <v-select v-model="search.recipient" :options="userOptions" label="name" :reduce="user => user.id" placeholder="Select Recipient" />
              </b-form-group>
            </b-col>
          </b-row>
          <b-row>
        ...
        <div v-if="!modalLoading">
          ...
          >
            <template #cell(transaction_id)="data">
              <span class="font-weight-bold">{{ data.item.transaction_id }}</span>
            </template>
            ...
            <template #cell(status)="data">
              ...
              <div v-if="data.item.status === 'rejected' && data.item.reject_reason" class="text-danger small mt-1">
                Reason: {{ data.item.reject_reason }}
              </div>
            </template>
            
            <template #cell(image_path)="data">
              <b-link @click="showImage(data.item.image_path)" class="text-primary cursor-pointer">
                View Receipt
              </b-link>
            </template>
            ...
            <template #cell(actions)="data">
              <template v-if="data.item.status === 'pending' && isAdmin">
                <b-button size="sm" variant="success" @click="approveRequest(data.item)">
                  Approve
                </b-button>
                <b-button size="sm" variant="danger" @click="rejectRequest(data.item)" class="ml-1">
                  Reject
                </b-button>
              </template>
              <template v-else-if="!isAdmin && data.item.request_by === currentUserId && data.item.status === 'pending'">
                <b-button size="sm" variant="primary" @click="openRequestModal(data.item.id)" class="ml-1">
                  Edit
                </b-button>
                <b-button size="sm" variant="danger" @click="deleteRequest(data.item.id)" class="ml-1">
                  Delete
                </b-button>
              </template>
              <template v-else>
      ...
      <!-- Request Modal -->
      <b-modal v-model="showRequestModal" @hide="resetModal" centered size="lg"
        :title="isEdit ? 'Edit Money Request' : 'Create Money Request'" hide-footer>
        <div v-if="editmodalLoading && isEdit" class="d-flex justify-content-center align-items-center" style="height: 100px;">
          <i class="fas fa-spinner fa-spin fa-3x"></i>
        </div>
        <div v-if="!editmodalLoading">
          <b-form @submit.prevent="submitRequest">
            <b-row>
              <b-col md="6">
                <b-form-group label="Transaction ID" label-for="transaction_id">
                  <b-form-input id="transaction_id" v-model="requestData.transaction_id" required
                    placeholder="Enter transaction ID" @input="removeError('transaction_id')" />
              ...
              <b-col md="6">
                <b-form-group label="Amount (₹)" label-for="amount">
                  <b-form-input id="amount" type="number" v-model="requestData.amount" required min="1"
                    placeholder="Enter amount" @input="removeError('amount')" />
            ...
            <b-row>
              ...
              <b-col md="6">
                <b-form-group label="Payment Receipt" label-for="image">
                  <input type="file" @change="handleFileChange" id="image" accept="image/*" :required="!isEdit" />
                  ...
                  <div v-if="isEdit && requestData.image_path" class="mt-2">
                    <b-link @click="showImage(requestData.image_path)" class="text-primary cursor-pointer">
            ...
            <b-form-group label="Description" label-for="description">
              <b-form-textarea id="description" v-model="requestData.description" rows="3"
                placeholder="Optional description..." />
            </b-form-group>

            <div class="d-flex justify-content-end">
              <b-button variant="primary" :disabled="loading" @click="submitRequest">
                <span v-if="loading">
      ...
      <!-- Reject Reason Modal -->
      <b-modal v-model="showRejectModal" title="Reject Request" centered @ok="confirmReject">
        <b-form-group label="Reject Reason" label-for="rejectReason">
          <b-form-textarea id="rejectReason" v-model="rejectReason" rows="3" required
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'request_create_for', label: 'Recipient', formatter: (value, key, item) => item.recipient.name },
        { key: 'created_at', label: 'Request Date', sortable: true, formatter: value => new Date(value).toLocaleString() },
  ...
  methods: {
    ...
    openRequestModal(editId = null) {
      ...
      if (editId) {
        ...
        axios.get(`/money-requests/${editId}`)
          .then(response => {
            this.requestData = {
              transaction_id: response.data.transaction_id,
              amount: response.data.amount,
              description: response.data.description,
              request_create_for: response.data.request_create_for,
    ...
    approveRequest(request) {
      if (confirm(`Are you sure you want to approve this request?`)) {
        axios.patch(`/money-requests/${request.id}/status`, { status: 'approved' })
          .then(() => {
            this.fetchRequestss();
    ...
    confirmReject() {
      axios.patch(`/money-requests/${this.currentRequestToReject.id}/status`, { 
        status: 'rejected',
    ...
    deleteRequest(id) {
      if (confirm("Are you sure you want to delete this request?")) {
        axios.delete(`/money-requests/${id}`)
          .then(() => {
            this.fetchRequestss();
