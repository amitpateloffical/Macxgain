...
export default {
  ...
  methods: {
    ...
    async makeApiCall(endpoint, options = {}) {
      ...
      try {
        const response = await fetch(`${this.getApiBaseUrl()}${endpoint}`, requestOptions);
        
        // Check if response is HTML (API not configured)
    ...
    async loadBackupData() {
      ...
      try {
        ...
        const response = await this.makeApiCall('/api/backups', {
          method: 'GET'
        });
    ...
    async createBackup() {
      ...
      try {
        const response = await this.makeApiCall('/api/backups/create', {
          method: 'POST',
          body: JSON.stringify({
    ...
    async downloadBackup(backupId) {
      try {
        const response = await this.makeApiCall(`/api/backups/${backupId}/download`, {
          method: 'GET',
          headers: {
    ...
    async restoreBackup(backupId) {
      ...
      if (backup) {
        if (confirm(`Are you sure you want to restore backup: ${backup.name}? This will overwrite current data.`)) {
          try {
            const token = localStorage.getItem('access_token');
            const response = await this.makeApiCall(`/api/backups/${backupId}/restore`, {
              method: 'POST'
            });
    ...
    async deleteBackup(backupId) {
      ...
      if (backup) {
        if (confirm(`Are you sure you want to delete backup: ${backup.name}?`)) {
          try {
            const token = localStorage.getItem('access_token');
            const response = await this.makeApiCall(`/api/backups/${backupId}`, {
              method: 'DELETE'
            });
...
.btn-create-backup {
  ...
  align-items: center;
  gap: 8px;
  box-shadow: 0 4px 16px rgba(var(--color-primary-rgb, 255, 215, 0), 0.3);
}

.btn-create-backup:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 8px 24px rgba(var(--color-primary-rgb, 255, 215, 0), 0.4);
}
...
.status-card:hover {
  ...
  box-shadow: 0 12px 32px rgba(var(--color-primary-rgb, 255, 215, 0), 0.2);
  opacity: 1;
}
...
.backup-item:hover {
  border-color: var(--color-primary, #FFD700);
  transform: translateY(-2px);
  box-shadow: 0 8px 24px rgba(var(--color-primary-rgb, 255, 215, 0), 0.1);
  opacity: 1;
}
...
.backup-modal {
  ...
  box-shadow: 0 20px 60px rgba(var(--color-primary-rgb, 255, 215, 0), 0.2);
  opacity: 0.98;
}
...
.option-card-modal.active {
  border-color: var(--color-primary, #FFD700);
  background: linear-gradient(145deg, rgba(var(--color-primary-rgb, 255, 215, 0), 0.1), rgba(var(--color-primary-rgb, 255, 215, 0), 0.05));
  box-shadow: 0 8px 24px rgba(var(--color-primary-rgb, 255, 215, 0), 0.3);
}
...
.toast-success {
  border-color: var(--color-primary, #FFD700);
  background: linear-gradient(145deg, rgba(var(--color-primary-rgb, 255, 215, 0), 0.1), var(--color-bg-secondary, #101022));
}
