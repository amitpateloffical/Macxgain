...
if command -v mysql >/dev/null 2>&1; then
    ...
    if mysql -u root -e "USE macxgain;" 2>/dev/null; then
        echo "✅ Macxgain database exists"
    else
        echo "📊 Creating Macxgain database..."
        mysql -u root -e "CREATE DATABASE IF NOT EXISTS macxgain;"
        mysql -u root -e "CREATE USER IF NOT EXISTS 'macxgain'@'%' IDENTIFIED BY 'macxgain123';"
        mysql -u root -e "GRANT ALL PRIVILEGES ON macxgain.* TO 'macxgain'@'%';"
        mysql -u root -e "CREATE USER IF NOT EXISTS 'macxgain'@'localhost' IDENTIFIED BY 'macxgain123';"
        mysql -u root -e "GRANT ALL PRIVILEGES ON macxgain.* TO 'macxgain'@'localhost';"
